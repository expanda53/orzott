...
OKiadas.prototype.nextHkodGet = function (result){
	...
	for (var i = 0;i < result.length;i++){
        ...
        if (res.HKOD=='NOTFOUND') {
            showMessage('Nincs több szedhető tétel!','');
            kiadas.showReview();
	...
	for (var i = 0;i < result.length;i++){
		...
		if (res.RESULTTEXT=='OK') {
			...
			switch (res.RESULTTEXT) {
				case 'NOT_FOUND': 
					errormsg='Nem található ilyen rendszám a kiszedendő abroncsok között!';
					break;
				case 'ALREADY_DONE': 
				...
				case 'UNKNOWN_ERROR': 
					...
					break;					
				case 'NOT_READY': 
					errormsg='Az előző rendszám még nincs kész! ('+kiadas.lastRsz+')';
					break;					
				default:
					errormsg = res.RESULTTEXT;
